...
describe('tokenize', function() {
    ...
    it('should tokenize dotted lookups', function() {
        assert.deepEqual(
            tokenize('foo.bar'),
            [{type: "UnquotedIdentifier", value: "foo", start: 0, end: 3},
             {type: "Dot", value: ".", start: 3, end: 4},
             {type: "UnquotedIdentifier", value: "bar", start: 4, end: 7},
            ]);
    });
    it('should tokenize numbers', function() {
        assert.deepEqual(
            tokenize('foo[0]'),
            [{type: "UnquotedIdentifier", value: "foo", start: 0, end: 3},
             {type: "Lbracket", value: "[", start: 3, end: 4},
             {type: "Number", value: 0, start: 4, end: 5},
             {type: "Rbracket", value: "]", start: 5, end: 6},
            ]);
    });
    it('should tokenize numbers with multiple digits', function() {
        assert.deepEqual(
            tokenize("12345"),
            [{type: "Number", value: 12345, start: 0, end: 5}]);
    });
    it('should tokenize negative numbers', function() {
        assert.deepEqual(
            tokenize("-12345"),
            [{type: "Number", value: -12345, start: 0, end: 6}]);
    });
    ...
    it('should tokenize literal lists with chars afterwards', function() {
        assert.deepEqual(
            tokenize("`[0, 1]`[0]"), [
                {type: "Literal", value: [0, 1], start: 0, end: 8},
                {type: "Lbracket", value: "[", start: 8, end: 9},
                {type: "Number", value: 0, start: 9, end: 10},
                {type: "Rbracket", value: "]", start: 10, end: 11}
        ]);
    });
    it('should tokenize two char tokens with shared prefix', function() {
        assert.deepEqual(
            tokenize("[?foo]"),
            [{type: "Filter", value: "[?", start: 0, end: 2},
             {type: "UnquotedIdentifier", value: "foo", start: 2, end: 5},
             {type: "Rbracket", value: "]", start: 5, end: 6}]
        );
    });
    it('should tokenize flatten operator', function() {
        assert.deepEqual(
            tokenize("[]"),
            [{type: "Flatten", value: "[]", start: 0, end: 2}]);
    });
    ...
    it('should tokenize two char tokens without shared prefix', function() {
        assert.deepEqual(
            tokenize("=="),
            [{type: "EQ", value: "==", start: 0, end: 2}]
        );
    });
    it('should tokenize not equals', function() {
        assert.deepEqual(
            tokenize("!="),
            [{type: "NE", value: "!=", start: 0, end: 2}]
        );
    });
    it('should tokenize the OR token', function() {
        assert.deepEqual(
            tokenize("a||b"),
            [
                {type: "UnquotedIdentifier", value: "a", start: 0, end: 1},
                {type: "Or", value: "||", start: 1, end: 3},
                {type: "UnquotedIdentifier", value: "b", start: 3, end: 4}
            ]
        );
    });
    it('should tokenize function calls', function() {
        assert.deepEqual(
            tokenize("abs(@)"),
            [
                {type: "UnquotedIdentifier", value: "abs", start: 0, end: 3},
                {type: "Lparen", value: "(", start: 3, end: 4},
                {type: "Current", value: "@", start: 4, end: 5},
                {type: "Rparen", value: ")", start: 5, end: 6}
            ]
        );
    });
...
describe('parsing', function() {
    it('should parse field node', function() {
        assert.deepEqual(compile('foo'),
                          {type: 'Field', name: 'foo', start: 0, end: 3});
    });
});
